  ...
  db.query(
    ...
    (err, students) => {
      if (err) {
        ...
        return res.status(500).json({ error: "Error al buscar estudiantes en la base de datos" });
      }
      ...
      if (students.length > 0) {
        ...
        db.query(
          ...
          (err, pagos) => {
            if (err) {
              ...
              return res.status(500).json({ error: "Error al buscar los pagos del alumno en la base de datos" });
            }
        ...
        res.status(404).json({ error: "No se encontró ningún alumno con ese nombre" });
      }
    }
  );
};
